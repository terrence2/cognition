...
static void trans_insertion_sort( int length );
...
static const vec2 spr_tc0 = { 0.0f, 0.0f };
static const vec2 spr_tc1 = { 1.0f, 0.0f };
static const vec2 spr_tc2 = { 1.0f, 1.0f };
static const vec2 spr_tc3 = { 0.0f, 1.0f };
...
void d_DrawTransparent()
{
	...
	// sort
	trans_insertion_sort( iNumTransObjs );
...
void trans_insert_sort_inner( int offset, trans_struct_t* value ) 
{
	int i = offset - 1;
	while( i >= 0 && tPtrs[i]->rsqrd > value->rsqrd ) {
		tPtrs[i + 1] = tPtrs[i];
		i--;
	}
	tPtrs[i + 1] = value;
...
void trans_insertion_sort( int length ) 
{
	int i = 1;
	while( i < length ) {
		trans_insert_sort_inner( i, tPtrs[i] );
		i++;
	}
}
