...
// This was originally built on Win32 with lots of (somewhat accurate) thoughts
// about an eventual port to linux.  This was originally constructed for the
// full file to be ported, but in actuallity, that would be insane.  Instead we
// have moved this to a service and moved the porting to a base layer to 
// abstract the sockets stuff.
...
int net_sv_Flush( connection_t client );
...
#ifdef WIN32
	// call the ever so short-bus "special" windows startup stuff
...
#else
    wsaData = 0; // prevent silly warning messages
    ret = 0;
#endif
...
byte net_cl_Connect( char *address, char *port )
{
	...
	if( (destAddr.sin_addr.s_addr = inet_addr( address )) == INADDR_NONE )
	{
...
#else
    on = 1; // prevent silly compiler warnings
...
int net_cl_GetServerPacket( byte *type, unsigned short int *size, byte **data )
{
	...
	while( !pkType && (cnt < 4) )
	{
		...
		{
			// an error occured that may mean many things, we need to check it carefully
            int err = WSAGetLastError();
            if( WOULD_HAVE_OVERRUN( err ) ) {
                // set the frame parameters to match this condition, then continue with the operation
				clRcFrLen += length; 
            }
            else if( IS_FINISHED_FOR_NOW( err ) ) {
                return 0;
            }
            else if( IS_FATAL_ERROR( err ) ) {
            ...
            else if( IS_MEANINGLESS_WARNING( err ) ) {
				con_Print( "Network Warning:  Client receive failed with a meaningless error." );
            }
            else {
				con_Print( "<RED>Network System Error:	An unknown error has occured in the network system." );
				con_Print( "<RED>The program will now exit.  Please report this error and help squash this bug." );
				eng_Stop( "34698" );
				return -1;
            }
...
int net_cl_Flush()
{
	...
	while( (clSdFrLen > 0) && (cnt < 4) )
	{
		...
		if( iThisSend == SOCKET_ERROR )
		{
            // an error occured that may mean many things, we need to check it carefully
            int err = WSAGetLastError();
            if( IS_FINISHED_FOR_NOW( err ) || SEND_BUFFER_FULL( err ) ) {
                return 1;
            }
            else if( WOULD_HAVE_OVERRUN( err ) || IS_FATAL_ERROR( err ) ) {
				con_Print( "<RED>Network System Error:	A fatal error has occured in the network system." );
				con_Print( "<RED>The program will now exit.  Please report this error and help squash this bug." );
				eng_Stop( "34698" );
				return -1;
...
#ifdef WIN32
	ioctlsocket( svConnections[index], FIONBIO, &on );
#else
    on = 1; // prevent silly compiler warnings
    fcntl( svConnections[index], F_SETFL, O_NONBLOCK );
#endif
