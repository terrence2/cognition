...
typedef struct
{
	...
	byte version PACKED;
	char name[MOO_NAME_SIZE] PACKED;
	uint32_t num_tags PACKED;
	uint32_t num_meshes PACKED;
	uint32_t num_frames PACKED;
	uint32_t frames_per_second PACKED;
	double frames_per_millisecond PACKED;
	float hibounds[3] PACKED;
	float lobounds[3] PACKED;
	float radius PACKED;
	uint32_t file_size PACKED;
	uint32_t header_size PACKED;
} moo_file_header_t;

typedef struct
{
	char name[MESH_NAME_SIZE] PACKED;
	int num_verts PACKED; // and # norms, and # map coords
	int num_tris PACKED;
	int num_mverts PACKED;
	int num_strip_lists PACKED;
	int mesh_size PACKED;	// size from first byte of header to end of sphere tree data
	int mesh_header_size PACKED;
...
static moo_data_t *moo_lm_GetMooHeader( moo_file_header_t *header, file_t *fp )
{
    ...
    // read the header
    ret = fs_read( &(header->magic), 4, 1, fp );
    if( ret != 1 ) {
        con_Print( "Moo Load Error:  Read Failed." );
        return 0;
    }
    
    ret = fs_read( &(header->version), 1, 1, fp );
    if( ret != 1 ) {
        con_Print( "Moo Load Error:  Read Failed." );
        return 0;
    }

    ret = fs_read( &(header->name), 1, 64, fp );
    if( ret != 64 ) {
        con_Print( "Moo Load Error:  Read Failed." );
        return 0;
    }

    // windows repacks here
    fs_read( buffer, 3, 1, fp );
    
    ret = fs_read( &(header->num_tags), 4, 1, fp );
    if( ret != 1 ) {
        con_Print( "Moo Load Error:  Read Failed." );
        return 0;
    }

    ret = fs_read( &(header->num_meshes), 4, 1, fp );
    if( ret != 1 ) {
        con_Print( "Moo Load Error:  Read Failed." );
        return 0;
    }
    
    ret = fs_read( &(header->num_frames), 4, 1, fp );
    if( ret != 1 ) {
        con_Print( "Moo Load Error:  Read Failed." );
        return 0;
    }
    ...
    ret = fs_read( &(header->frames_per_millisecond), 8, 1, fp );
    if( ret != 1 ) {
        con_Print( "Moo Load Error:  Read Failed." );
        return 0;
    }
    ...
    ret = fs_read( &(header->hibounds), 4, 3, fp );
    if( ret != 3 ) {
        con_Print( "Moo Load Error:  Read Failed." );
        return 0;
    }

    ret = fs_read( &(header->lobounds), 4, 3, fp );
    if( ret != 3 ) {
        con_Print( "Moo Load Error:  Read Failed." );
        return 0;
    }
    
    ret = fs_read( &(header->radius), 4, 1, fp );
    if( ret != 1 ) {
        con_Print( "Moo Load Error:  Read Failed." );
        return 0;
    }
    
    ret = fs_read( &(header->file_size), 4, 1, fp );
    if( ret != 1 ) {
        con_Print( "Moo Load Error:  Read Failed." );
        return 0;
    }

    ret = fs_read( &(header->header_size), 4, 1, fp );
    if( ret != 1 ) {
        con_Print( "Moo Load Error:  Read Failed." );
        return 0;
    }

    // why?  because window packs structures on 4 byte boundaries
    fs_read( buffer, 4, 1, fp );


/*
	uint32_t num_tags PACKED;  // set to 0
	uint32_t num_meshes PACKED; // invert
	uint32_t num_frames PACKED; // invert
	uint32_t frames_per_second PACKED; // invert
	double frames_per_millisecond PACKED; // ignore
	float hibounds[3] PACKED; // ignore
	float lobounds[3] PACKED; // ignore
	float radius PACKED; // ignore
	uint32_t file_size PACKED;  // wtf?
	uint32_t header_size PACKED;  // wtf?
*/

/*
    con_Print( "MAGIC: 0x%X", header->magic );
    con_Print( "VERSION: %d", header->version );
    con_Print( "NAME: %s", header->name );
    con_Print( "NUM TAGS: %d", header->num_tags );
    con_Print( "NUM MESHES: %d", header->num_meshes );
    con_Print( "NUM FRAMES: %d", header->num_frames );
    con_Print( "FPS: %d", header->frames_per_second );

    con_Print( "HIBOUND[0]: %f", header->hibounds[0] );
    con_Print( "HIBOUND[1]: %f", header->hibounds[1] );
    con_Print( "HIBOUND[2]: %f", header->hibounds[2] );
    con_Print( "LOBOUND[0]: %f", header->lobounds[0] );
    con_Print( "LOBOUND[1]: %f", header->lobounds[1] );
    con_Print( "LOBOUND[2]: %f", header->lobounds[2] );
    con_Print( "RADIUS: %f", header->radius );
    
    con_Print( "FILE SIZE: %d", header->file_size );
    con_Print( "HEADER SIZE: %d", header->header_size );
    con_Print( "REAL HEADER SIZE: %d", sizeof(moo_file_header_t) );
*/
    
    // check the header
    if( header->magic != MOO_FILE_MAGIC_NUMBER ) {
		con_Print( "Moo Load Error:  File is not a moo file." );
		return NULL;
    }
    
    if( header->version != MOO_FILE_VERSION ) {
		con_Print( "Moo Load Error:  Wrong version." );
		return NULL;
    }

    if( header->header_size != (sizeof(moo_file_header_t) + 7) ) {
...
/*
	con_Print( "MESH NAME: %s", mesh_header->name );
	con_Print( "MESH NUM VERTS: %d", mesh_header->num_verts );
	con_Print( "MESH NUM TRIS: %d", mesh_header->num_tris );
	con_Print( "MESH NUM MVERTS: %d", mesh_header->num_mverts );
	con_Print( "MESH NUM STRIP LISTS: %d", mesh_header->num_strip_lists );
	con_Print( "MESH SIZE: %d", mesh_header->mesh_size );
	con_Print( "MESH HEADER SIZE: %d", mesh_header->mesh_header_size );
*/	
