...
{
	byte imageIDLength;  // size of additional data after header
	byte colorMapType;  // always 0
	byte imageTypeCode;  // uncompressed BGR(A) is 2 ; uncompressed grayscale is 3
	short int colorMapOrigin;  // always 0
	short int colorMapLength;  // always 0
	byte colorMapEntrySize; // always 0
	short int imageXOrigin; // always 0
	short int imageYOrigin; // always 0
	short int imageWidth; // width of image in pixels (lo-hi)
	short int imageHeight; // height of image in pixels (lo-hi)
	byte bitCount; // bpp - 16, 24, or 32
	byte imageDescriptor; // 24 bit = 0x00;  32 bit = 0x08
} TARGAFILEHEADER;
