...
void d_TextLineBlack( char *line, float font_size, float pos_x, float pos_y )
{
	...
	for( a=0 ; a < length ; a++ )
	{
		...
		if( (console.cs_bnds_map[line[a]] != lastMap) )
		{
			lastMap = console.cs_bnds_map[line[a]];
			SAFE_BIND( console.charset[lastMap] );
		}

		pos += d_TextChar( line[a], font_size, console.cs_bnds_scale[console.cs_bnds_map[line[a]]], pos, pos_y ) + DEF_INSPACE;
	}
}
...
static float d_TextChar( char c, float font_size, float xScale, float pos_x, float pos_y )
{
	...
	width = console.cs_bnds_width[c] * xScale * font_size / 12.0f;
	height = console.cs_height * font_size / 12.0f; // pre scaled

	glBegin( GL_QUADS );

		glTexCoord2f( console.cs_bnds_lo[c], 0 );
		glVertex2f( pos_x, pos_y );

		glTexCoord2f( console.cs_bnds_hi[c], 0 );
		glVertex2f( pos_x + width, pos_y );

		glTexCoord2f( console.cs_bnds_hi[c], 1 );
		glVertex2f( pos_x + width, pos_y + height );

		glTexCoord2f( console.cs_bnds_lo[c], 1 );
...
float d_GetTextWidth( char *text, float font_size )
{
	...
	for( a = 0 ; a < length ; a++ )
	{
		...
		fTotWidth += DEF_INSPACE + (console.cs_bnds_width[text[a]] * console.cs_bnds_scale[console.cs_bnds_map[text[a]]] * font_size / 12.0f);
	}
...
float d_GetCharWidth( char c, float font_size )
{
	return DEF_INSPACE + (console.cs_bnds_width[c] * console.cs_bnds_scale[console.cs_bnds_map[c]] * font_size / 12.0f);
}
