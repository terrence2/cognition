...
static int eng_Initialize()
{
	...
	if( !t_Initialize() ) return 0; // so that calls to mem_alloc will have times in debug builds
...
// Timers and Timing Data
	ei->t_GetTime = t_GetTime;
	ei->t_GetSaneTime = t_GetSaneTime;
