...
void ps_Draw()
{
	...
	while( ps != NULL )
	{
		...
		for( a = 0 ; a < ps->uiNumParticles ; a++ )
		{
			...
			assert( part != NULL );
			trans_Sprite( part->vPos, part->fSize, part->fAng, part->color, part->shape );
		}
